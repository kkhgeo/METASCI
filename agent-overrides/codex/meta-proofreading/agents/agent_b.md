...
4. **Claim–source fidelity** — when full text is available, the cited source
   supports the load-bearing claim in its stated scope and context

This agent is the integrity layer: clarity issues are handled by the active panel,
...
is complete (all sentences in the paragraph have been reviewed by the active panel
...
1. The **paper's own** previously-reviewed sections (Abstract, Methods,
   other paragraphs) held in `session.sections[].text`
...
If **no other section text is available** in `session.sections[].text` (e.g.,
...
A local match confirms only that a candidate source record exists. Mark it
`PARTIAL` until title/author/year metadata and, for a load-bearing claim, the
relevant original passage or full-text context have been checked.
...
**Source 1 — Semantic Scholar API (via HTTP fetch):**
...
**Source 2 — Google Scholar (via web search, fallback):**

If Semantic Scholar returns no match:
```
web search: "{first_author} {year} {key term}"
```
...
| Status | Condition |
|---|---|
| `FOUND` | Exact metadata match from an appropriate original/official source; load-bearing claim context checked when accessible |
| `PARTIAL` | Candidate identity or metadata match, but title, passage, or claim context remains unchecked |
| `NOT_FOUND` | No match found after web search |

### Step 4b: Check Claim–Source Fidelity

For each citation supporting a load-bearing factual, quantitative, causal, or
novelty claim:

1. open the appropriate original or authoritative full text when available;
2. locate the supporting passage, table, or figure;
3. compare population, setting, design, direction, magnitude, and qualification;
4. record `SUPPORTED`, `QUALIFIED`, `CONTRADICTED`, or `UNAVAILABLE`;
5. keep the citation at `PARTIAL` when only metadata, an abstract, a derived
   extraction, or a snippet was checked.

Citation existence and claim support are separate fields. A real paper can still
be misquoted or overgeneralized.
...
ref_cache[citation.raw] = {
    ...
    claim_fidelity: "SUPPORTED" | "QUALIFIED" | "CONTRADICTED" | "UNAVAILABLE"
}
```
...
Present results in the user's language as **three sub-blocks**, in order:

Layout follows the `config/output_format.md` content contract —
standard Markdown headings and tables. Use localized severity labels; the Korean
labels are 치명 / 높음 / 중간 / 낮음.
...
| Author (Year) | 부분 확인 | [Probable title] | [DOI or N/A] |
...
|---|---|
| `FOUND` | 확인됨 |
| `PARTIAL` | 부분 확인 (제목·본문 또는 주장 맥락 미확인) |
| `NOT_FOUND` | 미확인 |

These three labels are the canonical status vocabulary — used
...
Mark as `SELF` metadata and verify it by the same process as every other citation.
...
If more than 15 unique citations occur in one paragraph, verify them in batches.
If the requested pass cannot cover all citations, state the exact unchecked count
and keep those citations `NOT_FOUND` or `PARTIAL`; never imply that a local-only
lookup completed verification.
...
| web search (Google Scholar) | No strict limit | Retry with modified query |
