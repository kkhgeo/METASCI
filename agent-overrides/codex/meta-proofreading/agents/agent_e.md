...
(`{project_root}/`). Then search for directories matching:
...
**Trigger condition:** `knowledge_bank.quality.sufficient == false` for the current
claim or review decision.

If available sources lack direct, authoritative, or independent coverage,
supplement with web search.
If the user has opted out ("웹검색 없이"), skip this phase entirely.

**Search strategy (follow `knowledge/search_strategy.md`):**

**Source 1 — Google Scholar (via web search):**

Generate 2-3 short queries (3-6 words each):
```
Query 1: "[target_journal] [keyword1] [keyword2]"
Query 2: "[keyword1] [keyword2] [keyword3] [field-appropriate date range if needed]"
Query 3: "[keyword1] [keyword2] review OR meta-analysis"
```

**Source 2 — Semantic Scholar API (via HTTP fetch):**
...
4. Date relevance for the specific claim (recent is not automatically better)
...
R4 and R5 carry no knowledge by design (persona-only reviewers). Activate them
only when the mode-sensitive panel size and review risk justify those perspectives.
See `knowledge/distribution_strategy.md` for the allocation order.
...
Present in the user's language (Korean example below):
...
quality = {
    ...
    sufficient: bool,   // claim- and task-specific assessment
