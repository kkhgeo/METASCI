...
Phase A: 의도 확인 (사용자 승인)
  ↓
Phase B: 리뷰어 패널 1회전 (이슈 + 실질적으로 개선 가능한 단위의 후보 생성)
...
Decision tools may be used when available; the diagnosis and full alternatives
remain in the message body. These typed commands are always honored:
...
| User says | Action |
|---|---|
| "최적안 전부 적용" | Apply the judge's optimal for every reviewed unit at once (ORIGINAL selections stay unchanged) |
