...
Each presented issue follows the `config/output_format.md` content contract,
localized to the user's language. The Korean labels are
원문 → 문제 → 조치(directive) → 근거 → 발견자.
...
**User action:** choice tool when available, or typed `"[#] 적용"`.
...
**User action:** choice tool when available (options: `R1 따름` / `R4 따름` /
`직접 입력` / `건너뛰기`).
...
Pool consensus, unique, and conflict findings, then sort globally by:

1. severity and downstream consequence;
2. evidence quality and verification state;
3. scope of the affected text;
4. independent corroboration as a tiebreaker.

Category remains visible as metadata; it is not the first sort key. Since all
reviewers share one model and one instruction set, consensus measures salience
more than truth. A supported HIGH finding raised by one reviewer outranks a LOW
finding raised by five.
...
Reviewers return, from ONE panel round: CANDIDATE sets for materially improvable
units, explicit ORIGINAL nominations for clean units they assess, ISSUES, and
...
Score each issue by **impact**, then sort descending. Impact combines severity,
evidence quality, downstream scope, and only then independent agreement:

```
impact_score =
    severity_weight (CRITICAL=8, HIGH=6, MEDIUM=3, LOW=1)
  + evidence_weight (original/official=3, strong contextual=2, heuristic=1)
  + downstream_scope (cross-section=3, paragraph=2, sentence=1)
  + independent_agreement (capped at 1)
```

**Why agreement is capped at 1.** R1–R5 run the same model on identical
...
Severity, evidence quality, and scope must dominate; agreement is a tiebreaker.
