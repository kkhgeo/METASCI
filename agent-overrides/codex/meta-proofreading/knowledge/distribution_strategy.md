...
> agreement bonus at 1 and why `agents/agent_j.md` Rule 5 judges candidates on
...
Set panel size from the mode before activating allocations:

- sentence: 2 reviewers;
- paragraph: 2–3 reviewers;
- section: 3–4 reviewers;
- paper: 3–5 reviewers.

Increase within the range only for material domain, factual, or structural risk.
The cases below define a maximum allocation pool, not a mandatory reviewer count.
Activate the selected number in this diversity order: R1, R2, R5, R3, R4.
...
Maximum allocation pool: 5; activate only the mode-appropriate panel size.
```
...
Maximum allocation pool: 4; activate only the mode-appropriate panel size.
```
...
Maximum allocation pool: 3; activate only the mode-appropriate panel size.
```

R4 and R5 need no knowledge files. Include an outside-subfield or generic
perspective when the selected panel size and review risk justify it; they are not
