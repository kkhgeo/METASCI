...
다음 순서로 하나의 스타일을 확정하고 작업 기록에 남긴다.

1. 사용자가 명시한 스타일
2. 현재 목표 투고처가 요구하는 스타일
3. 투고처가 미확정이거나 별도 요구가 없을 때 APA 7 fallback

형식은 달라져도 인용-참고문헌 매칭, Claim-Source 내용 일치, 원문 확인 원칙은 동일하다.
아래 APA 7 예시는 fallback 구현 예시이며 투고처 규정보다 우선하지 않는다.
...
**Knowledge Sources (선행연구)** — 반드시 선택된 단일 인용 스타일로 표기
...
## 2. Reference List 형식 (APA 7 fallback 예시)

투고처 스타일이 확정되었으면 이 절의 APA 예시 대신 투고처의 본문 인용, 정렬,
저자 표기, 문헌 유형별 서식을 적용한다.
...
### Step 2: 선택된 인용·참고문헌 형식 검증

```
작업:
□ 적용 스타일과 근거 기록: 사용자 지정 / 투고처 지침 / APA 7 fallback
□ 본문 인용, 정렬, 저자 표기, 문헌 유형별 형식이 선택된 스타일과 일치
□ 필수 메타데이터 완비 여부 확인
□ APA 7 fallback이면 저자명, 연도, 저널명·권, DOI 형식을 이 문서 §2와 대조
```
...
### Step 5: 단면 간 무결성 및 AI 근거 검증

```
Methods-Results:
  - 각 분석·절차에 대응 결과가 있는지 확인
  - 각 결과에 선행 Methods 근거가 있는지 확인
  - 누락·사후분석은 삭제하지 말고 명시적으로 표시

Introduction-Discussion:
  - Discussion의 중심 답변이 Introduction의 연구질문과 같은 범위인지 확인
  - 답변이 Results에 실제 보고된 근거에 의존하는지 확인

Abstract:
  - 본문·표·그림을 보지 않아도 목적, 방법, 핵심 결과, 결론을 이해할 수 있는지 확인

AI evidence inventory:
  - AI가 생성·변환한 사실, 수치, 인용마다 원출처와 검증 상태 기록
  - 미확인 항목은 `[unverified claim]`으로 유지
  - 문법·서식 지원 후에도 숫자와 인용이 이동·변형되지 않았는지 확인
```
...
- [ ] Citation style and authority documented (user / target venue / APA 7 fallback)
- [ ] Reference format validated against the selected style
...
- [ ] In-text citations use one uniform selected style (no * / † source-type markers)
...
- [ ] Discussion answers the Introduction's primary question at the same scope
- [ ] The Abstract stands alone without body, figure, or table dependencies
- [ ] AI-generated or AI-transformed facts, numbers, and citations have recorded verification status
- [ ] Current venue requirements were checked, or venue-dependent checks are marked provisional

**한국어:**
- [ ] 모든 본문 인용이 참고문헌에 존재
- [ ] 고아 참고문헌 없음
- [ ] 인용 스타일과 근거가 기록됨 (사용자 / 투고처 / APA 7 fallback)
- [ ] 참고문헌 형식이 선택된 스타일과 대조 검증됨
- [ ] 참고문헌에 누락 필드 없음
...
- [ ] 본문 인용이 선택된 단일 형식으로 통일됨 (소스타입 */† 마커 없음)
- [ ] 소스 유형은 내부 검증용으로만 기록 (본문·References 미표기)
- [ ] Web 검색 결과 신뢰성 확인
- [ ] DOI/URL/연도/저자 조작 없음
- [ ] 소스 원문 문장을 따옴표 없이 그대로 옮긴 곳 없음
- [ ] 모든 인과·기전 진술이 인용 근거에 매핑됨
- [ ] 모든 Methods 항목과 Results 항목이 양방향으로 대응됨
- [ ] Discussion의 중심 답변이 Introduction의 연구질문과 같은 범위임
- [ ] Abstract가 본문·그림·표 없이 독립적으로 이해됨
- [ ] AI가 생성·변환한 사실·수치·인용의 검증 상태가 기록됨
- [ ] 최신 투고처 요건을 확인했거나 미확정 항목을 잠정으로 표시함
...
| 문제 | 처리 | 사용자 확인 |
|------|------|------------|
| 선택된 인용 형식 오류 (기존 정보 재배열) | 자동 재포맷 (새 사실 추가 없음) | 불필요 |
