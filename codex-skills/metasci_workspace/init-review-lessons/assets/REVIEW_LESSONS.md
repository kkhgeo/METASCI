...
judgment leaves one line here, so the same point is not missed, and the
same declined proposal is not re-argued, the next time.

Two copies exist. The local copy in the manuscript folder holds every line
that applies to this manuscript. The global copy at
`Z:\KKH_Research\META_SCI\REVIEW_LESSONS.md` holds only the lines that
apply to every manuscript. A new manuscript folder is seeded from the
global copy.
...
3. **Decide the scope.** Terminology, journal format, this study's design,
   ...
   would hold for another manuscript → 전역.
4. **Check for a duplicate before appending.** Read the existing lines. If
   one already says the same thing, do not add another: raise its count
   marker (`×2`, `×3`) and update its date, in both copies for a 전역 line.
   A rising count means a rule keeps being missed; say so to the author
   when a count reaches 3.
5. **Append.** 이논문 lines go to the local copy only. 전역 lines go to
   the local copy **and** the global copy, identically. Append under the
   matching heading below. Never rewrite or delete existing lines on your
   own. If the global copy is unreachable (drive not mounted), append
   locally, add `[전역 미반영]` at the end of the line, and tell the author;
   on a later run, push those lines to the global copy and drop the mark.
6. **Report** the lines added or bumped, in one short list.
...
stays out of context. It is opened in two cases only:

- **A review comes in.** Read it to run the procedure above; the duplicate
  check in step 4 needs the existing lines.
- **The author asks for the lessons** — "교훈 봐줘", "리뷰 교훈 적용해서",
  `@REVIEW_LESSONS.md`, or similar. Then read both lists below and apply
  them, alongside the writing skills' own checklists, to the piece of work
  at hand. 확인할 것 is the set of checks past reviews found missing.
  받지 않는 제안 is the set of proposals already declined, with the reason.

## Maintenance

When either list passes about 50 lines, say so and offer to merge
near-duplicates. Merging is the author's call; never prune on your own.
<!-- /metasci:review-lessons -->
