...
- **Runtime:** Codex CLI; reviewers run as parallel sub-agents
  (one **`spawn_agent`** call per reviewer, all in a single response)
...
- **Tools used:** file reading, shell commands, web search, HTTP fetch, `spawn_agent`
- **Output:** `config/output_format.md` (v13) defines a **content
  ...
  line on every screen, and a numbered decision prompt at decision points.
...
Ask for approval via a numbered decision prompt (options: "이대로 진행" /
...
parallel — one `spawn_agent` call per reviewer, all in a single response.
...
Confirm via a numbered decision prompt (options: "맞아요" / "다름 — 직접 설명").
...
3. Walk through, one at a time, **only the sentences that either got a
   ...
   - display: previous sentence (context) → current sentence → Korean
     ...
     message body**, not compressed into a numbered decision prompt labels:
   ...
   - **선정 신뢰도 MEDIUM/LOW → 메뉴 자동 펼침.** Show Agent J's full
     ...
     plus why confidence is low. a numbered decision prompt: "N번 적용" / "원문 유지" /
