...
### 1. Google Scholar (via the `web` toolset)
...
### 2. Semantic Scholar API (via the `web` toolset)
...
### 3. Open Access Full Text (via the `web` toolset)
...
| web fetch (full text) | Per-domain | Skip, use abstract |
