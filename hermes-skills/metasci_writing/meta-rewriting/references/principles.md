...
> **2026-07-23 개정에서 이 파일은 의도적으로 변경하지 않았다.** 원리 은행(27개 소스) 대조 결과 이 파일에 추가할 후보가 나왔으나(원칙 8 Parallelism의 개념 층위 확장 등), 이 문서의 가치는 위 검증 방식 — 22개 소스, 3중 독립 검증 — 이 보증하는 출처 신뢰도에 있다. 검증 절차를 거치지 않은 항목을 섞으면 그 보증이 무너진다. 따라서 은행에서 온 항목은 전부 `section-checklists.md`에 배치하고, 이 파일의 원칙을 참조하는 형태로 연결했다.
>
> 대조에서 확인된 사실 하나: 이 파일의 핵심인 old-before-new(원칙 1), topic/stress position(원칙 2), topic string 진단법(원칙 5)에 **대응하는 은행 항목은 0개**다. 은행은 실무 지침서·트윗 계열이라 처방적 문체 규칙(간결·능동·평이)에 치우쳐 있고, 독자 인지처리 계열 원리는 이 파일에만 있다. 겹치는 지점에서는 언제나 이 파일이 더 정밀하므로 충돌 시 이 파일이 우선한다.
...
**Version**: 1.1.0 (원칙 본문은 1.0.0과 동일 — 위 개정 주석만 추가)
