...
| Korean personal voice / institutional register | `meta-mywriting-korean` / `meta-report-writing` |
...
draft whose prose is Korean is not, and goes to `meta-mywriting-korean` or
`meta-report-writing`. Measured: a live draft returned `lang: ko` on the raw file and `en`
...
| Korean draft or corpus | Stop; point to `meta-mywriting-korean` / `meta-report-writing` |
