...
- **Runtime:** Codex CLI. The interactive session acts as the
  **orchestrator**; reviewers run as parallel **`codex exec` child
  processes** (one isolated process per reviewer — see section 3a)
...
- **Tools used:** file reading, shell commands (reviewer spawning via
  `codex exec`, temp files), web search
- **Output:** `config/output_format.md` (v13) defines a **content
  ...
  line on every screen, and a numbered decision prompt at decision
  points. Any formatting example elsewhere in this skill is
  illustrative — when in doubt, `config/output_format.md` wins.
...
Agent E and Agent B run **inline in the orchestrator session** (they are
sequential, single-instance roles — no isolation needed). Only the
reviewers (R1-R5) run as isolated child processes, because reviewer
independence is what makes the deliberation meaningful.
...
## 3a. Reviewer Execution — `codex exec` Child Processes

Each reviewer run is one isolated, single-shot `codex exec` process.
Reviewers must NOT be simulated inside the orchestrator's own context —
each reviewer sees only its own prompt, never the conversation, never
another reviewer's material or output.

**Per panel round:**

1. **Compose prompts.** For each active reviewer Rn, fill the template
   from `agents/agent_reviewer.md` with `{mode}`, `{target_text}`,
   `{allocated_knowledge}`, `{writing_manual_content}` (and
   `{confirmed_intent}` in Mode 3). Inline all allocated content as
   text — the child process must not need to hunt for files. Write each
   prompt to a temp file (absolute path): `<tmp>/mp_prompt_Rn.md`.

2. **Launch ALL reviewers before reading ANY result** — one background
   process per reviewer, then wait for all:

   PowerShell (Windows):
   ```powershell
   $tmp = "$env:TEMP\meta-proofreading"
   $jobs = foreach ($r in $activeReviewers) {   # e.g. 'R1','R3','R4','R5'
     Start-Job -ArgumentList $r, $tmp {
       param($r, $tmp)
       Get-Content -Raw "$tmp\mp_prompt_$r.md" |
   ...
   bash (macOS/Linux):
   ```bash
   for r in "${active_reviewers[@]}"; do
     codex exec --skip-git-repo-check --ephemeral -s read-only \
       -o "$tmp/mp_out_$r.md" - < "$tmp/mp_prompt_$r.md" &
   done
   wait
   ```

   Flags: `--ephemeral` (no session persisted), `-s read-only`
   (reviewers only read), `-o` (final message captured to file),
   `-` (prompt from stdin — avoids command-line length limits).

3. **Collect.** Read each `mp_out_Rn.md`; parse the ISSUES /
   SUGGESTIONS / CONFIDENCE blocks defined in
   `agents/agent_reviewer.md`. A missing, empty, or unparseable output
   file counts as reviewer failure — handle per
   `config/session_management.md` Error Handling (retry once, then
   proceed with the remaining reviewers, minimum 2).

**One panel round per unit.** Never launch a second round for the same
paragraph/section — Mode 3's sentence walkthrough works entirely from
the single round's results.
...
- If a `.docx` or `.pdf`: convert to text first (use a converter such as
...
Follow the full Agent E workflow from `agents/agent_e.md` (inline, in
the orchestrator session):

1. **Phase 1:** Scan project Knowledge/Logic/Vocab directories.
   For each file, read the first 20 lines only.
...
Ask for approval with numbered options ("이대로 진행" /
...
parallel — one `codex exec` child process per reviewer, all launched
before any result is read (section 3a).
...
Launch all active reviewers in parallel (one `codex exec` process each,
section 3a) with:
- `{mode}` = `section`
...
Confirm with numbered options ("맞아요" / "다름 — 직접 설명").
...
3. Walk through **only the sentences that have findings**, one at a
   ...
   - decision via numbered options — pick the 4 most relevant,
...
Agent B following `agents/agent_b.md` (inline, in the orchestrator
session):
