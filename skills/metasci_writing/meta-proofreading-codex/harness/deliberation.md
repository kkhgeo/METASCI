...
All reviewers run in **parallel** (one `codex exec` child process per reviewer — see SKILL.md 3a).
...
**User action:** numbered options, or typed `"[#] 적용"` /
`"[#] [A/B/C] 적용"`.
...
**User action:** numbered options ( `R1 따름` / `R4 따름` /
`직접 입력` / `건너뛰기`).
