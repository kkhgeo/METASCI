...
| manual/INDEX.md | Skill directory | Routing table only |
...
| manual files from the section's INDEX.md routing row | `manual/INDEX.md` Step 1 table | Section file + that section's L1·L3 파일 — the routing table is the single source of truth |
| Knowledge files matched to section | Knowledge Bank | Reviewer knowledge |

**Do NOT load:** Other sections' text, L1·L3 파일 not in the
...
already-loaded manual + paragraph context plus the in-session
candidate pool.
...
| manual section file | Already loaded from Mode 2 | Reuse |
| ALL L1·L3 files (`L1_sentence_rules.md`, `L1_paragraph_rules.md`, `L3_style_cohesion.md`) | Skill directory | The single round also reviews at sentence depth |
...
```
R1: [manual files] + [Knowledge Group A files]
R2: [manual files] + [Knowledge Group B files]
R3: [manual files only]
...
Mode 2 entry:
  ...
  6. Read section-specific manual
