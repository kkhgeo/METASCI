...
|---|---|---|
| **R1** | manual + Knowledge Group A | Domain expert A |
| **R2** | manual + Knowledge Group B | Domain expert B |
| **R3** | manual only | Rule-based judge |
...
R1 → manual + Group A (content knowledge)
R2 → manual + Group B (writing patterns)
R3 → manual only
R4 → nothing                  (generic academic-writing reviewer persona)
R5 → nothing                  (cross-disciplinary scientific reader persona)
...
```
R1 → manual + whatever is available
R2 → manual only (becomes second baseline)
...
- If a reviewer's group has no matching entries for this paragraph,
  they still review with their base allocation (manual or none)
