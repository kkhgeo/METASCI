...
| 판단하려는 것 | 읽을 파일 | 위치 |
|---|---|---|
| 섹션끼리 아귀가 맞나 (전체 조망) | `structural-integrity.md` | 이 폴더 (번들) |
| 이 섹션에 무슨 단락이 있어야 하나 | `section-checklists.md` | 이 폴더 (번들) |
| 단락 순서·연결이 자연스러운가 | `cohesion_flow.md` | Glob `**/cross_section/cohesion_flow.md` |
...
번들 두 개는 `meta-proofreading`과 `meta-rewriting`에서 가져온 사본이다(로컬
설치본에 없어서 넣었다). 원본이 갱신되면 다시 복사한다.
