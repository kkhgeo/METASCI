...
- **★는 먼저 꺼낼 것.** 한 응답에 문제 지적은 최대 3개 — 그중 무엇을
  고를지의 기준이다.
...
## 회수분 — 설계 단계에서 판정 가능한 것 (2026-08-10)

`by-source/` 재채굴에서 나온 항목 중 **문장 없이도 판정되는 것**만 추렸다.
전체는 `meta-rewriting/references/section-checklists.md`, 근거·인용은
`claude_writing_manual/`에 있다.

**서론**
- 첫 단락이 "이 분야가 얼마나 중요한가"가 아니라 "이 논문이 무엇에 관한
  것인가"를 알리는가 ★ — 분야 예찬으로 시작하면 결함 [Schulzrinne]
- 자기 연구실·시스템의 개발 내력으로 시작하지 않는가 ★ [Schulzrinne]
- 서론이 초록을 다시 쓰고 있지 않은가 — 섹션 간 중복 [Schulzrinne]
- Related Work가 한 곳에만 있는가 — 서론 뒤 또는 요약 직전 중 택일,
  양쪽 중복 금지 [Schulzrinne]
- 목표가 사슬로 엮여 있지 않은가 — 한 목표가 다른 목표의 *특정 결과*에
  의존하면 결함. 심사자 주시 항목 ★ [IUFRO]
- 목표(구체·시한)와 목적(추상)이 섞이지 않았는가 [IUFRO]

**문헌고찰**
- 유사 조건에서 이미 수행된 연구의 반복이 아님을 리뷰가 보이는가 ★ —
  게재 거부의 명시적 사유 [IUFRO Ch.9]
- (리뷰 논문) 포함·제외 범위가 서론에 밝혀져 있는가 [Koons 2019]
- 논문 단위가 아니라 아이디어 단위로 묶였는가, 공통 흐름 → 분기 순서인가
  [Hon (UF)]

**초록**
- (임상시험) CONSORT 필수 항목과 등록번호가 들어갈 자리가 있는가 ★ —
  규정 사항 [Tullu 2019]
- 키워드 3–10개가 제목 단어와 다르게 배치되어 있는가 [Tullu 2019]
- Nature 7부 템플릿을 쓴다면 부분별 독자 수위가 지켜지는가 — 도입은 어느
  분야 과학자든, 상세 배경은 인접 분야 [Nature B7]

**결과**
- 표·그림 묶음마다 도입 문단이 배치되어 있는가 [Fisher et al.]

**논의**
- 서론 마지막 문단과 구조가 대응하는가 ★ — 세 목표를 세웠으면 셋을
  논의하는가, 가설을 세웠으면 채택·기각을 논증하는가 [Fisher et al.]
- 반론·대안적 설명을 다루는 자리가 있는가 [Lund]

**미해결 3건은 설계 단계에서 결정한다** — 초록 길이, 제목 길이, 초록의 태.
셋 다 투고처가 정하는 문제이므로, 구성 합의 시점에 대상 저널을 확인해 기록해
두면 집필·교정 단계에서 다시 흔들리지 않는다. `claude_writing_manual/DECISIONS.md`

1인칭과 단정 vs 헤징은 **2026-08-10에 해결됐다** — 선택지로 꺼내지 않는다.
1인칭은 결정·해석·논증에 쓰고, 주장 강도는 중간("suggests", "indicates")으로
간다. 경위: `claude_writing_manual/DIAGNOSTIC-MAP.md`
